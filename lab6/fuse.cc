...
// register sig handler for version control
void sig_handler(int no) {
    switch (no) {
        case SIGINT:
            printf("[version]commit a new version\n");
            yfs->commit();
            break;
        case SIGUSR1:
            printf("[version]to previous version\n");
            yfs->rollback();
            break;
        case SIGUSR2:
            printf("[version]to next version\n");
            yfs->forward();
            break;
    }
}
