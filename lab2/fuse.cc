...
//
void fuseserver_setattr(fuse_req_t             req,
                        fuse_ino_t             ino,
                        struct stat           *attr,
                        int                    to_set,
                        struct fuse_file_info *fi) {
...
void fuseserver_create(fuse_req_t req, fuse_ino_t parent, const char *name,
    ...
    if ((ret =
             fuseserver_createhelper(parent, name, mode, &e,
...
int reply_buf_limited(fuse_req_t req, const char *buf, size_t bufsize,
    ...
    if ((size_t)off <
        bufsize) return fuse_reply_buf(req, buf + off,
                                       min(bufsize - off, maxsize));
    else return fuse_reply_buf(req, NULL, 0);
}
...
void fuseserver_mkdir(fuse_req_t req, fuse_ino_t parent, const char *name,
    ...
    if ((ret =
             fuseserver_createhelper(parent, name, mode, &e,
                                     extent_protocol::T_DIR)) == yfs_client::OK) {
        fuse_reply_entry(req, &e);
    } else {
        if (ret == yfs_client::EXIST) {
            fuse_reply_err(req, EEXIST);
