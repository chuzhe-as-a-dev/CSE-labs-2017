...
char abstract_byte::encode(char data) {
    bool data0 = data & 0x1;
    bool data1 = data & 0x2;
    char encoded = data0 << 7 | data0 << 6 | data0 << 5 |
                   data1 << 4 | data1 << 3 | data1 << 2 |
                   (data0 ^ data1) << 1 | (data0 ^ data1);
    return encoded;
}


char abstract_byte::decode(char data) {
    bool bit0 = data & 0x1;  // data0 ^ data1
    bool bit1 = data & 0x2;  // data0 ^ data1
    bool bit2 = data & 0x4;  // data1
    bool bit3 = data & 0x8;  // data1
    bool bit4 = data & 0x10;  // data1
    bool bit5 = data & 0x20;  // data0
    bool bit6 = data & 0x40;  // data0
    bool bit7 = data & 0x80;  // data0

    bool data0_corrupted = bit5 != bit6 || bit6 != bit7;
    bool data1_corrupted = bit2 != bit3 || bit3 != bit4;
    bool parity_corrupted = (bit0 != bit1) || (!data0_corrupted && !data1_corrupted && (bit0 != bit2 ^ bit5));
    corrupted |= data0_corrupted || data1_corrupted || parity_corrupted;  // don't overwrite previous corruption

    if (!data0_corrupted && !data1_corrupted) {  // both are fine
        return bit2 << 1 | bit5;
    } else if (data0_corrupted && data1_corrupted) {  // both are damaged
        return vote(bit2, bit3, bit4) << 1 | vote(bit5, bit6, bit7);
    } else if (parity_corrupted) {  // one data and one parity are damaged
        return vote(bit2, bit3, bit4) << 1 | vote(bit5, bit6, bit7);
    } else {  // only data is damaged, 1 or 2 bit flip(s)
        if (data0_corrupted) {
            return bit2 << 1 | (bit0 ^ bit2);
        } else {
            return (bit0 ^ bit5) << 1 | bit5;
        }
    }
}


bool abstract_byte::vote(bool bit0, bool bit1, bool bit2) {
    if (bit0 == bit1) {
        return bit0;
    } else if (bit0 == bit2) {
        return bit0;
...
abstract_byte::abstract_byte(char data) {
    corrupted = false;

    actual = data;
    coded0 = encode(data & 0x3);
    coded1 = encode((data & 0xc) >> 2);
    coded2 = encode((data & 0x30) >> 4);
    coded3 = encode((data & 0xc0) >> 6);
}

abstract_byte::abstract_byte(char data0, char data1, char data2, char data3) {
    corrupted = false;  // corrupted is set to true inside decode

    char decoded0 = decode(data0);
    char decoded1 = decode(data1);
    char decoded2 = decode(data2);
    char decoded3 = decode(data3);
    actual = decoded0 & 0x3 |
             (decoded1 & 0x3) << 2 |
             (decoded2 & 0x3) << 4 |
             (decoded3 & 0x3) << 6;
    ...
    if (corrupted) {
        coded0 = encode(decoded0);
        coded1 = encode(decoded1);
        coded2 = encode(decoded2);
        coded3 = encode(decoded3);
    } else {
        coded0 = data0;
        coded1 = data1;
        coded2 = data2;
...
void block_manager::read_block(blockid_t bnum, char *buf) {
    ...
    // read coded data on disk
    char coded[BLOCK_SIZE * 4];
    d->read_block(bnum * 4, coded);
    d->read_block(bnum * 4 + 1, coded + BLOCK_SIZE);
    d->read_block(bnum * 4 + 2, coded + BLOCK_SIZE * 2);
    d->read_block(bnum * 4 + 3, coded + BLOCK_SIZE * 3);

    // check each byte for corruption
    bool corrupted = false;
    for (size_t i = 0; i < BLOCK_SIZE; i++) {
        abstract_byte byte(coded[i * 4], coded[i * 4 + 1], coded[i * 4 + 2], coded[i * 4 + 3]);
        buf[i] = byte.actual;

        if (byte.corrupted) {
            coded[i * 4] = byte.coded0;
            coded[i * 4 + 1] = byte.coded1;
            coded[i * 4 + 2] = byte.coded2;
            coded[i * 4 + 3] = byte.coded3;
            corrupted = true;
        }
    }

    // save corrected data, if any
    if (corrupted) {
        d->write_block(bnum * 4, coded);
        d->write_block(bnum * 4 + 1, coded + BLOCK_SIZE);
        d->write_block(bnum * 4 + 2, coded + BLOCK_SIZE * 2);
        d->write_block(bnum * 4 + 3, coded + BLOCK_SIZE * 3);
    }
}
